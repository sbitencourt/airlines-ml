...
def get_source_config(data_source: str) -> tuple[str, str]:
    ...
    return (
        "/flights/latest",
        "PostgreSQL contains ML-enriched flight predictions generated by the batch pipeline. "
        "This source includes current flight status plus predicted delay risk.",
    )
...
def infer_flight_status_message(row: pd.Series) -> str:
    ...
    prediction_label = get_display_value(row, ["prediction_label"], default="")
    delay_probability = get_display_value(row, ["delay_probability"], default="")

    if prediction_label and prediction_label != "Not available":
        try:
            probability = float(delay_probability)
            return (
                f"The flight is currently reported as '{status}'. "
                f"The ML model predicts: {prediction_label} "
                f"with {probability:.1%} delay probability."
            )
    ...
    departure_delay = get_display_value(
        row,
        ["departure.delay", "departure_delay", "departure_delay_min"],
        default="0",
    )
    arrival_delay = get_display_value(
        row,
        ["arrival.delay", "arrival_delay", "arrival_delay_min"],
        default="0",
    )
