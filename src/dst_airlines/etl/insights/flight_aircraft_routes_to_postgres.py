...
from dst_airlines.insights.flight_aircraft_routes import build_flight_aircraft_routes
from dst_airlines.io.postgres_writer import insert_flight_aircraft_routes
...
def build_aircraft_routes_to_postgres(
    source: str = "aviationstack",
    endpoint: str = "flights",
    run_id: str | None = None,
) -> None:
    if not run_id:
        raise ValueError("run_id is required to build aircraft route rows")

    source = source.strip().lower()
    endpoint = endpoint.strip().lower()

    stage = "flight_aircraft_routes"
    ...
    for file_path in target_files:
        ...
        routes = build_flight_aircraft_routes(
            payload=payload,
            run_id=run_id,
            source=source,
            endpoint=endpoint,
        ...
        insert_flight_aircraft_routes(routes)
        total_inserted += len(routes)

        log_event(
            "INFO",
            stage,
            "aircraft_routes_inserted",
            run_id=run_id,
            source=source,
            endpoint=endpoint,
            file_name=file_path.name,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build flight aircraft route rows and write them to PostgreSQL"
    )
    ...
    build_aircraft_routes_to_postgres(
        source=args.source,
